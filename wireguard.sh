...
new_client_dns() {
	...
	case "$dns" in
		...
		;;
		4)
			dns="2620:0:ccc::2, 208.67.222.222"
		;;
		5)
			dns="2620:fe::fe, 9.9.9.9"
		;;
		6)
			dns="2a10:50c0::ad1:ff, 94.140.14.14"
		;;
		7)
			enter_custom_dns
			if [ -n "$dns2" ]; then
...
PresharedKey = $psk
AllowedIPs = 10.7.0.$octet/32$(grep -q '2001:470:48f6:d::1/64' /etc/wireguard/wg0.conf && echo ", 2001:470:48f6:d::$octet/128")
# END_PEER $client
EOF
...
[Interface]
Address = 10.7.0.$octet/32$(grep -q '2001:470:48f6:d::1/64' /etc/wireguard/wg0.conf && echo ", 2001:470:48f6:d::$octet/128")
DNS = $dns
PrivateKey = $key
MTU = 1342
...
[Interface]
Address = 10.7.0.1/24$([[ -n "$ip6" ]] && echo ", 2001:470:48f6:d::1/64")
...
EOF
	...
	if systemctl is-active --quiet firewalld.service; then
		...
		if [[ -n "$ip6" ]]; then
			firewall-cmd -q --zone=trusted --add-source=2001:470:48f6:d::/64
			firewall-cmd -q --permanent --zone=trusted --add-source=2001:470:48f6:d::/64
			firewall-cmd -q --direct --add-rule ipv6 nat POSTROUTING 0 -s 2001:470:48f6:d::/64 ! -d 2001:470:48f6:d::/64 -j ACCEPT
			firewall-cmd -q --permanent --direct --add-rule ipv6 nat POSTROUTING 0 -s 2001:470:48f6:d::/64 ! -d 2001:470:48f6:d::/64 -j ACCEPT
		fi
	else
...
ExecStop=$iptables_path -D FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT" > /etc/systemd/system/wg-iptables.service
		if [[ -n "$ip6" ]]; then
			echo "ExecStart=$ip6tables_path -t nat -A POSTROUTING -s 2001:470:48f6:d::/64 ! -d 2001:470:48f6:d::/64 -j ACCEPT
ExecStart=$ip6tables_path -I FORWARD -s 2001:470:48f6:d::/64 -j ACCEPT
ExecStart=$ip6tables_path -I FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT
ExecStop=$ip6tables_path -t nat -D POSTROUTING -s 2001:470:48f6:d::/64 ! -d 2001:470:48f6:d::/64 -j ACCEPT
ExecStop=$ip6tables_path -D FORWARD -s 2001:470:48f6:d::/64 -j ACCEPT
			...
			if [[ "$remove" =~ ^[yY]$ ]]; then
				...
				if systemctl is-active --quiet firewalld.service; then
					...
					if grep -qs '2001:470:48f6:d::1/64/64' /etc/wireguard/wg0.conf; then
						...
						firewall-cmd -q --direct --remove-rule ipv6 nat POSTROUTING 0 -s 2001:470:48f6:d::/64 ! -d 2001:470:48f6:d::/64 -j ACCEPT
						firewall-cmd -q --permanent --direct --remove-rule ipv6 nat POSTROUTING 0 -s 2001:470:48f6:d::/64 ! -d 2001:470:48f6:d::/64 -j ACCEPT
					fi
				else
